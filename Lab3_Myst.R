...
Regla0_R <- -.045  # Considerar una oportunidad de compra en un rendimiento de -6% o menor.
...
npm <- 21
pondt <- runif(npm, min=0, max=1)
pond <- pondt/sum(pondt)
Regla1_I <- .2   # Porcentaje de capital para comprar titulos para posicion Inicial.
Regla2_P <- .15   # Se utiliza el P% del L capital restante en cada compra.
...
for(i in (npm+1):(length(Historico$Date))){
  ...
  if(BorS(Historico$Precio,npm,pond,i)){ #se activa una se??al
    ...
    if(Historico$Capital[i] > 0){ # Si hay capital
      if(Historico$Capital[i]*Regla2_P > Historico$Precio[i]*Historico$Capital[i]*Regla2_P){ # Si Capital minimo
